...
rpv_ellipse_data <- list(
  `11` = map(table_2, \(x) ellipse.pv.rpv(pv.rpv(x$data_table$y1y2), exponentiate = TRUE)),
  `15` = map(table_2, \(x) ellipse.pv.rpv(pv.rpv(x$data_table$y1y3), exponentiate = TRUE))
)

#imap(rpv_ellipse_data, \(x, y) plot_rpv_ellipse(x, y))
plot_rpv_ellipse_overlay(rpv_ellipse_data)
```
...
ggplot(septa, aes(x = psa, y = psa_a3p)) +
  geom_point(pch = 21, cex = 2, alpha = 0.7) +
...
#| fig-width: 6
#| fig-height: 6
ggplot(septa, aes(x = 0.5*(log2(psa)+log2(psa_a3p)), y = log2(psa)-log2(psa_a3p))) +
  geom_point(pch = 21, cex = 2, alpha = 0.5) +
  geom_hline(yintercept = 0, col = "red", lwd = 0.7) +
  geom_smooth(method = "loess", se = FALSE, col = "deepskyblue", lwd = 1.2) +
  scale_y_continuous(limits = c(-8, 8), n.breaks = 8) +
  theme_minimal(base_size = 12) +
  labs(x = "A = log2(PSA * PSA for Stockholm3)/2",
       y = "M = log2(PSA / PSA for Stockholm3)",
