...
tables_list$table_1 <- septa |>
  ...
  select(race, race_copy, age, psa, risk_score, dre_abnormal, fhpca, prevbx, five_ari, 
         risk_score, targeted_biopsy_performed, pirads3p, pirads4p, piradsns, cb_cores_total, 
...
# Relative PPV and NPV for Stockholm≥15 vs PSA≥4 ng/ml
...
rpv_ellipse_data <- map(table_2, \(x) ellipse.pv.rpv(pv.rpv(x$data_table$y1y3), exponentiate = TRUE))

plot_rpv_ellipse(rpv_ellipse_data)
```
...
tables_list$table_1_biobank <- septa |>
  make_table1_vars() |> 
  mutate(strata_septa = factor(!grepl("^northwestern|^uhn", centre_id), labels = make_factor_labels()$ny)) |> 
...
bloodtest_crosstable <- 
  map(bloodtests[2:3], \(p)
      map(bloodtests[4:5], \(s) {
        ...
        footnote <- paste0("Cohen's kappa coefficient: ", myfmt(kappa), ".")
        ...
        # print as gt for html purposes...
        print(xtab |> as_gt() |> tab_footnote(footnote)) 
        
        #...but export as flextable to save further down 
        # exporting tbl_cross tables with gtsave corrupts the whole docx
        xtab |>
          as_flex_table() |>
...
if(params$save_tables == "true") {
  which_bloodtest <- grep("^bloodtest_crosstable_", names(tables_list))
  filename_out <- here::here("output", paste0(params$time, "-septa_tables_study1.docx"))
                         
  gt_group(.list = tables_list[-which_bloodtest]) |> 
    gtsave(filename_out)
  
  Sys.sleep(5)
  
  # exporting tbl_cross tables with gtsave corrupts the whole docx
  doc <- read_docx(filename_out)
  walk(tables_list[which_bloodtest], \(x) {
    doc <- body_add_break(doc)
    doc <- flextable::body_add_flextable(doc, value = x)
  })
  print(doc, target = filename_out)
